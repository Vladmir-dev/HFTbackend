...
class TradeAlgo:
    ...
    def trade(self,symbol, qty):
        if self.market_open:
            ...
            #List of daily high
            barset = self.api.get_barset(symbol, 'day', limit=self.lookback)
            ...
            if not self.position and symbol_close_price >= max(self.high):
                ...
                print(f"submitting {self.side} order for {qty} of {symbol}")
            ...
            #update the stop price/ trail_price
                ...
                self.stopMarketTicket = self.api.submit_order(symbol=symbol, qty=qty, side=self.side, type=self.type, trail_price=self.trail_price, time_in_force=self.time_in_force)
                print(f"submitting {self.side} order for {qty} of {symbol}")
